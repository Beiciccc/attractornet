...
alternatives all \emph{saturate} (Fig.~\ref{fig:driving})---a free-input ``activatable
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{fig_driving.pdf}
\caption{Only the quiescent condition is non-degenerate. Mean per-net fraction of genes
reachable on the real BBM networks under four driving conditions; the quiescent
inputs-OFF task we study (green) is the only one that avoids saturation, so it is the
principled rather than a cherry-picked choice.}
\label{fig:driving}
\end{figure}
