...
\thanks{Manuscript prepared June 2026. Code, synthetic generators, and trained models
...
signalling dynamics; biologically meaningful behaviour is captured by attractors and
by reachability between states, both hard to compute exactly in general. Classical theory
relates a BN's interaction graph to its dynamics only through necessary conditions
and bounds: the wiring diagram provably under-determines the attractor set. We turn
this into a well-posed learning problem with free, exact supervision. Using an exact
symbolic oracle for ground-truth labels, we (i) quantify that a predictor restricted
to graph topology is at chance (\auroc{} $0.56$), because fixing the wiring and
varying only the Boolean functions flips the attractor label for $84$--$88\%$ of
wirings; and (ii) introduce a sign-aware, size-agnostic graph neural network (GNN)
that reads the local Boolean logic and predicts \emph{per-node asynchronous
activation reachability}---which genes can switch on from a quiescent state. In
distribution the GNN beats a gradient-boosted tree given \emph{all} truth-table bits,
specifically on reachability ($0.96$ vs $0.92$) and not on the more global has-cyclic
property---a controlled inductive-bias effect. Trained only on small synthetic BNs,
the GNN transfers \emph{zero-shot} to real curated gene-regulatory networks from the
biodivine-boolean-models collection, predicting per-node activation reachability at
pooled \auroc{} $0.91$ ($95\%$ CI $[0.88,0.93]$) and beating every size-agnostic
baseline, including a CANA effective-connectivity baseline whose local-feature mapping
does not transfer. A sign\,/\,no-sign\,/\,MLP ablation shows the gain is carried by
sign-aware message passing, not recomputed canalization. All labels come from exact symbolic
computation---no manual annotation---and all experiments run on a single GPU.
\end{abstract}
...
with cell phenotypes---and its \emph{reachability} structure: from a given state,
which configurations, and which individual gene activations, the dynamics can attain.
A recurring systems-biology question is which genes can be switched on from a
quiescent (all-OFF) state, and by what regulatory routes; this underlies reprogramming
and signal-response analysis.
...
conditions and bounds}, never determinations: the same signed interaction graph admits
update functions with one attractor or many, and deciding the count from the graph
alone is \#P-complete~\cite{pcomplete}. The interaction graph, in short, does not fix
the dynamics---the logic does.

We take this under-determination as the precondition for a \emph{learning} problem
(Fig.~\ref{fig:pipeline}). If the graph does not determine the dynamics, a model
predicting a dynamical property from the graph alone must be at chance---and any model
that succeeds must extract information from the \emph{logic}. This makes ``predict an
expensive dynamical property from structure-plus-logic'' a well-posed supervised task
with a built-in identifiability guarantee and a free, exact label source (a symbolic
oracle), requiring no manual annotation. We instantiate it for \emph{asynchronous
reachability}, a property that is PSPACE-hard to compute exactly in general, is
biologically central, and---unlike the mere existence of attractors---is fundamentally
...
property---a controlled inductive-bias result. A sign\,/\,no-sign\,/\,MLP ablation and
...
BN attractors from structure via spectral analysis---non-learned, targeting fixed-point
...
pre-computed by a separate procedure---a different task, with the dynamics handed to
...
\emph{has-cyclic-attractor}---a synchronous attractor of period $>1$ exists; (b)
\emph{attractor count}; and (c)---our headline---\emph{per-node activation
...
different logic variants of the same wirings in train and test. Crucially we
\emph{balance} the random/canalizing mix across the split: a naive variant-parity split
...
for $84\%$ of wirings (has-cyclic) and $88\%$ (reachability)---so on these instances
...
(Table~\ref{tab:identify}, Fig.~\ref{fig:1}). The signed graph (Thomas level) helps for
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{fig_1_identifiability.pdf}
\caption{The interaction graph is at chance; the logic decides. Within-graph \auroc{}
by feature group for has-cyclic and asynchronous reachability.}
\label{fig:1}
\end{figure}
...
for which a sign-aware encoder is the natural and, as we show, load-bearing choice. We use
...
regulators activate or inhibit it---information a sign-agnostic aggregator discards. The
ablation in \S\ref{sec:ablation} confirms this term is load-bearing.
...
\emph{has-cyclic} property ($0.751$ vs $0.803$, Fig.~\ref{fig:4}). This selective win is
the signature of an inductive bias, not raw capacity: message passing captures the
propagation structure of reachability, whereas a feature-interaction tree does as well
or better on a property that is not propagation-shaped. The GNN is thus the right tool
specifically for reachability.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\columnwidth]{fig_4_taskspecific.pdf}
\caption{The GNN wins only where propagation matters: it beats the all-bits tree on
asynchronous reachability but not on the global has-cyclic property.}
\label{fig:4}
\end{figure}
...
(median $0.99$, IQR $[0.85,1.00]$, mean $0.90$; Fig.~\ref{fig:3}); we report the pooled
...
$\rho=-0.14$, $p=0.14$; Fig.~\ref{fig:3}, left). The tail instead comprises networks whose
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_3_pernet_transfer.pdf}
\caption{Zero-shot transfer holds across network sizes ($118$ real GRNs). Left: per-net
\auroc{} vs size; right: distribution with median $0.99$ and mean $0.90$.}
\label{fig:3}
\end{figure}

\subsection{The gain is sign-aware propagation, not canalization}
\label{sec:ablation}
A natural concern is that reachability is recomputable from known \emph{local} logic
features (canalization, effective connectivity), so that transfer is parasitic on real
GRNs being heavily canalizing. We refute this two ways (Table~\ref{tab:ablation},
Fig.~\ref{fig:2}).
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_2_ablation_cana.pdf}
\caption{Sign-aware propagation, not canalization. In-distribution and zero-shot
transfer \auroc{} for the GNN variants and the canalization baselines; the CANA tree
anti-transfers below chance.}
\label{fig:2}
\end{figure}
...
\emph{not} spontaneously activatable from quiescence (Table~\ref{tab:vignette})---qualitatively
...
alternatives all \emph{saturate}---a free-input ``activatable under some signal'' variant,
a fully-stimulated condition (inputs fixed ON; mean per-net base $0.82$), and a
silenceability condition (reaching $0$ from the all-ON state; mean base $0.93$) each make
almost every gene reachable, leaving little to predict. The quiescent condition is thus
not cherry-picked for learnability but is the one biologically meaningful condition that
yields a non-degenerate label; characterizing the framework under richer, non-saturating
multi-signal conditions is important future work.
There is a genuine synthetic$\to$real distribution shift (synthetic activation base rate
$0.86$ vs real $0.34$); the GNN transfers despite it---the central strength---but a
...
all of them in well under a second), so the contribution is a scientific one---logic
carries transferable reachability structure---rather than a computational speed-up; we
...
dynamics---a classical fact we turn into a well-posed learning problem with free, exact
supervision. A sign-aware, size-agnostic GNN reading the local logic predicts
asynchronous reachability, beats an all-truth-table-bits model precisely where
propagation matters, and---trained only on small synthetic networks---transfers
zero-shot to real gene-regulatory networks, outperforming every size-agnostic baseline,
including a canalization model that fails to transfer at all. The logic carries
learnable, transferable reachability structure that no graph-only model can recover, and
a propagation-matched architecture is what captures it.
...
networks are the public biodivine-boolean-models collection. Code and trained models
will be released upon acceptance.
